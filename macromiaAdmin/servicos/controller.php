...
function sevicoSelect($conexao){
	if(isset($_POST['eventos'])){ $eventos_str          = implode(",",$_POST['eventos']);      $eventos_array          = explode(",",$eventos_str); }else{ $eventos_array = " "; }
	...
	if(isset($_POST['cliente_email'])){ $email_cli 		= $_POST['cliente_email']; }
	if(isset($_POST['txt_data'])){    	$data_evento    = $_POST['txt_data']; } else{ $data_evento = " "; }
	if(isset($_POST['txt_horario'])){ 	$horario_evento = $_POST['txt_horario']; }else{ $horario_evento = " "; }
	if(isset($_POST['txt_cep'])){     	$cep_evento 	= $_POST['txt_cep'];  }else{ $cep_evento = " "; }
	if(isset($_POST['txt_estado'])){  	$loca_estado 	= $_POST['txt_estado']; }else{ $loca_estado = " "; }
	if(isset($_POST['txt_cidade'])){  	$loca_cidade 	= $_POST['txt_cidade']; }else{ $loca_cidade = " "; }
	...
	echo " ".$loca_cidade." ".$loca_ende." ".$loca_estado." ".$loca_num." <-- Fim <br/>";
	...
	$resultadoSql = inserirEvento($conexao,$event_enviar,$id_cli,$data_event_insert,$horario_evento,$cep_evento, $loca_num, $orc_min,$orc_max,$loca_cidade,$loca_estado, $loca_ende);
	
	if($resultadoSql){
		sendEmail($event_enviar,$email_cli);	
		echo "Cadastro deu certo!";
		//header("Location: index.php?r=login");
		header("Location: index.php?r=login&p=logar");
	}else{
