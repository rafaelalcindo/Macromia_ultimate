...
if($cli_admin != " "){
	$lista_cliente_count02 = listar_cliente02_count($conexao, $cli_admin);
	$pg					   = (isset($_GET['pg']))? (int)$_GET['pg'] : 1;
	$start				   = ($pg - 1) * $limite;
	
	$listar_cliente2	= listar_cliente02($conexao,$cli_admin,$start, $limite);
	...
	echo "</tbody>";
	echo "</table>";
	if($lista_cliente_count02 > 1 && $pg <= $lista_cliente_count02){
		echo "<nav aria-label='Page navigation'>
  					<ul class='pagination'>";
		for($i = 1; $i <= $lista_cliente_count02; $i  ){			
			echo "<li><a onclick='cliente_paginacao_01($i)' style='cursor:pointer;'>$i</a></li>";			
		}
