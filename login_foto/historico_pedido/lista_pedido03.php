...
if($servico != ' ' && $estado != ' ' && $cidade != ' ' && $id_foto != null){
	...
	$lista_cli_01_count = lista_cli_opt01_count($conexao,$servico,$estado,$cidade,$limite,$orc_min,$orc_max, $id_foto);
	$pg = (isset($_GET['pg']))? (int)$_GET['pg'] : 1;
	$start = ($pg - 1) * $limite;
	
	$lista_cli = lista_cli_opt01($conexao,$servico,$estado,$cidade,$start,$limite,$orc_min,$orc_max,$ordenar, $id_foto);
	...
	//	echo "<h3>No momento estamos sem eventos</h3>";
		...
		if($lista_cli_01_count > 1 && $pg <= $lista_cli_01_count){
			echo "<div class='clearfix colelem' id='u505568' style='margin-left: 50%; margin-top: 13%;'>";
	   	  
	   		if($pg > 1){

			    echo "<div class='rounded-corners clearfix grpelem' id='u505569-4' style='cursor: pointer;' onclick='paginacao_pedidos($pg - 1, true)' data-muse-uid='U505569' data-muse-type='txt_frame'>
	   		...
	   		if($pg < $lista_cli_06_count){
	   			...
	   			echo "
			        <div class='rounded-corners clearfix grpelem' id='u505570-4' style='cursor: pointer;' onclick='paginacao_pedidos($pg + 1, true)' data-muse-uid='U505570' data-muse-type='txt_frame'><!-- content -->
	...
	header('Content-type: text/html; charset=ISO-8859-1');
	$lista_cli_02_count = lista_cli_opt02_count($conexao,$servico,$estado, $limite,$orc_min,$orc_max, $id_foto);
	$pg = (isset($_GET['pg']))? (int)$_GET['pg'] : 1;
	$start = ($pg - 1) * $limite;
	
	$lista_cli02 = lista_cli_opt02($conexao,$servico,$estado, $start, $limite,$orc_min, $orc_max, $ordenar, $id_foto);	
	...
	$lista_cli_04_count = lista_cli_opt04_count($conexao,$estado, $cidade, $limite,$orc_min, $orc_max, $id_foto);
	$pg = (isset($_GET['pg']))? (int)$_GET['pg'] : 1;
	$start = ($pg - 1) * $limite;
	
	$lista_cli04 = lista_cli_opt04($conexao,$estado,$cidade,$start, $limite,$orc_min, $orc_max, $ordenar, $id_foto);
	...
	$lista_cli_05_count = lista_cli_opt5_count($conexao,$servico, $limite,$orc_min, $orc_max, $id_foto);
	$pg = (isset($_GET['pg']))? (int)$_GET['pg'] : 1;
	$start = ($pg - 1) * $limite;
	
	$lista_cli05 = lista_cli_opt05($conexao,$servico, $start, $limite,$orc_min, $orc_max, $ordenar, $id_foto);
	...
	$lista_cli_03_count = lista_cli_opt3_count($conexao,$estado, $limite,$orc_min, $orc_max, $id_foto); // conta quantas linhas tem na tabela
	$pg = (isset($_GET['pg']))? (int)$_GET['pg'] : 1;
	$start = ($pg - 1) * $limite;
	
	$lista_cli03 = lista_cli_opt03($conexao, $estado, $start, $limite,$orc_min, $orc_max, $ordenar, $id_foto);
	...
	//echo "<br/>Entrou no else 6, lista pedido 3 val estado".$estado." <br/>";
	//echo "Id_foto: ".$id_foto;
	$lista_cli_06_count = lista_cli_opt6_count($conexao, $limite,$orc_min, $orc_max, $id_foto);
	$pg = (isset($_GET['pg']))? (int) $_GET['pg'] : 1;
	$start = ($pg - 1) * $limite;

	$lista_cli06 = lista_cli_opt06($conexao, $start, $limite,$orc_min, $orc_max,$ordenar, $id_foto);
	   ...
	   if($lista_cli_06_count > 1 && $pg <= $lista_cli_06_count){
	   	  echo "<div class='clearfix colelem' id='u505568' style='margin-left: 50%; margin-top: 13%;'>";
	   		...
	   		if($pg < $lista_cli_06_count){
	   			...
	   			echo "
	   				<div class='rounded-corners clearfix grpelem' id='u505570-4' style='cursor: pointer;' onclick='paginacao_pedidos($pg + 1, true)' data-muse-uid='U505570' data-muse-type='txt_frame'>
			         <p id='u505570-2'><span id='u505570'>PRÓXIMO</span></p>
			        </div>
	   			";
